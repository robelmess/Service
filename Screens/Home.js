...
export default function HomeScreen({ navigation }) {
  ...
  function expandData(data) {
    ...
    for (; d <= end; d = new Date(d.getTime() + 24 * 60 * 60 * 1000)) {
      ...
      if ((dayOfWeek === 3 || dayOfWeek === 5) && !expandedData[newDate] && !excludedDates.includes(newDate)) {
        const dotColor = theme === 'light' ? 'black' : 'white';
        expandedData[newDate] = {
  ...
  const Legend = React.memo(({ month }) => {
    ...
    return (
      <View style={theme == 'light' ? styles.legendContainerLight : styles.legendContainerDark}>
        {legendItems.map((item, index) => (
          <View key={index} style={styles.legendColorDotWrapper}>
            ...
            />
            <Text style={theme == 'light' ? styles.legendColorLabelLight : styles.legendColorLabelDark}>{item.legendTag}</Text>
          </View>
        ))}
      </View>
    );
    ...
    <View style={{ flex: 1 }}>
      <View style={{ flex: 1 }}>
        <CalendarList
          ...
          theme={theme == 'light' ? styles.homePageLight : styles.homePageDark}
        />
        <View style={styles.legendWrapper}>
          <Legend month={currentMonth} />
        </View>
        
        <TouchableOpacity onPress={handleJumpToToday} style={theme == 'light' ? styles.jumpToTodayButtonLight : styles.jumpToTodayButtonDark}>
          <Text style={theme == 'light' ? styles.jumpToTodayTextLight : styles.jumpToTodayTextDark}>Today</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  jumpToTodayButtonTextLight: {
    color: '#000',
    fontSize: 16,
  },
  jumpToTodayButtonTextDark: {
    color: '#FFF',
    fontSize: 16,
  },
  legendTagContainer: {
  ...
  legendColorLabelLight: {
    fontSize: 12,
    color: 'black'
  },
  legendColorLabelDark: {
    fontSize: 12,
    color: 'white'
  },
  jumpToTodayTextLight: {
    fontSize: 16,
    color: 'black',
  },
  jumpToTodayTextDark: {
